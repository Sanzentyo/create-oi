//! Music example using `SerialTransport`.
//!
//! Demonstrates:
...
//!
//! ```text
//! cargo run --example songs_sync -- /dev/ttyUSB0
//! ```
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    create.define_song(SongNumber::new(0)?, &scale)?;

    println!("Playing song 0...");
    create.play_song(SongNumber::new(0)?)?;
    ...
    create.define_song(SongNumber::new(1)?, &fanfare)?;

    println!("Playing song 1...");
    create.play_song(SongNumber::new(1)?)?;
