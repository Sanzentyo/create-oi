...
impl<T: Transport> Create<Off, T> {
    ...
    pub fn reset(mut self) -> Result<T, ConnectError<T, std::io::Error>> {
...
impl<T: Transport> Create<Passive, T> {
    ...
    ///
    /// On Roomba 400, sends the CONTROL command (opcode 130), which is required
    /// to transition from Passive to Safe on that model. On Create 1 and Create 2,
    /// sends the SAFE command (opcode 131).
    ...
    /// create a new `Create::<Off, _>::new(transport, model)`.
...
impl<M: Mode, T: Transport> Create<M, T> {
    ...
    /// # Note on robot state
    ///
    /// Dropping the returned transport (or this `Create` handle directly) does
    ...
    /// [`stop()`](crate::mode::Actuatable) or [`power_off()`](crate::mode::SensorReadable)
    /// before releasing the handle if safe shutdown is needed.
    #[must_use]
    ...
    /// Validate that a sensor packet ID is supported by the current robot model.
    ///
    /// Group packets (0–6, 100) are checked against `RobotModel::supports_group_packet`.
    ...
    fn reject_if_not_streaming(&self) -> Result<(), Error<std::io::Error>> {
        if !self.streaming {
            return Err(Error::Validation(ValidationError {
                field: "stream",
    ...
    #[inline(always)]
    fn cleared_transition<N: Mode>(self) -> Create<N, T> {
        Create {
            transport: self.transport,
            model: self.model,
            stream_parser: StreamParser::new(),
